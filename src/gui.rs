...
use eframe::egui::{
    global_dark_light_mode_buttons, Align2, ColorImage, FontFamily, FontId, Pos2, Vec2, Visuals,
};
...
pub struct MyApp {
    ...
    prog_counter: String,
    prog_counter_lock: Arc<RwLock<String>>,
}
...
impl MyApp {
    ...
    fn console_text(&self, packet: &crate::data::Packet) -> Option<String> {
        ...
        if packet.payload.trim().starts_with("PC is ") {
            //return Some(packet.payload.clone().replace("PC is ", "") + "\n");
            self.prog_counter_lock.write().unwrap().clear();
            self.prog_counter_lock.write().unwrap().push_str(&packet.payload.clone().replace("PC is ", ""));
           // return None
        }
        ...
        egui::SidePanel::new(Side::Right, "settings panel")
            ...
            .show(ctx, |ui| {
                ui.add_enabled_ui(true, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(format!("no lock {}", self.prog_counter));
                        ui.add_space(5.0);
                        ui.label(format!("lock {:?}", self.prog_counter_lock.read()));
                    });
